...
function Cart ({show,handleClose,details}) {
    ...
    const payment= async(total)=>{
        try {
            ...
            const response = await axios.get(`${process.env.REACT_APP_API_URL}/paying?details=${detailsString}&total=${total}`, { withCredentials: true });
            if(response.data){
                pay();
            }
        } catch (error) {
    ...
    const deleteDetail= async(detailId)=>{
        try {
            const response= await axios.get(`${process.env.REACT_APP_API_URL}/deleteFromCart?detailId=`+detailId,{withCredentials:true});
            if(response.data){
                removeFromCart();
            }
        } catch (error) {
